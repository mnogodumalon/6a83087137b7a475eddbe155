...
for (const [slug, page] of surfacePages) {
  ...
  for (const ep of page.endpoints || []) {
    ...
    // file on EITHER side: /files is not grantable, so the service rejects
    // the whole page — reading (list) and uploading (create) alike. Checked
    // for every op, not just list: a live landing page lost its ingest to
    // three file fields and the gate, list-only back then, stayed silent.
    if (Array.isArray(ep.fields) && appMeta) {
      const controls = appMeta.apps?.[ep.entity]?.controls || {};
      for (const key of ep.fields) {
        if (controls[key]?.fulltype?.startsWith('file')) {
          errors.push(`${SURFACE}: page '${slug}' endpoint '${ep.entity}' exposes file field '${key}' — attachments cannot go public at all (neither listed nor uploaded); drop it, or hold the image in a string/url field`);
        }
      }
    }
