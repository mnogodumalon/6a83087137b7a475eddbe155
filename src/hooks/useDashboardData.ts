...
export function useDashboardData() {
  ...
  const [themenFeedback, setThemenFeedback] = useState<ThemenFeedback[]>([]);
  ...
  const fetchAll = useCallback(async () => {
    setError(null);
    try {
      const [mitgliederData, sitzungenData, anmeldungenData, protokolleData, themenFeedbackData, notizenData] = await Promise.all([
      ...
      setThemenFeedback(themenFeedbackData);
  ...
  return { mitglieder, setMitglieder, sitzungen, setSitzungen, anmeldungen, setAnmeldungen, protokolle, setProtokolle, themenFeedback, setThemenFeedback, notizen, setNotizen, loading, error, fetchAll, mitgliederMap, sitzungenMap, protokolleMap };
}