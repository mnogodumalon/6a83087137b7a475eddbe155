        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('tagesordnung')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('sitzungen', 'tagesordnung')}
                  {sortKey === 'tagesordnung' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('mitglieder')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('sitzungen', 'mitglieder')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('einladung_versendet')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('sitzungen', 'einladung_versendet')}
                  {sortKey === 'einladung_versendet' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('sitzungsstatus')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('sitzungen', 'sitzungsstatus')}
                  {sortKey === 'sitzungsstatus' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/sitzungen/${record.record_id}`); }}>
                ...
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.tagesordnung ?? '—'}</span></TableCell>
                ...
                <TableCell><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${record.fields.einladung_versendet ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>{record.fields.einladung_versendet ? t('yes') : t('no')}</span></TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{lookupLabel('sitzungen', 'sitzungsstatus', record.fields.sitzungsstatus?.key) ?? record.fields.sitzungsstatus?.label ?? '—'}</span></TableCell>
                <TableCell>
