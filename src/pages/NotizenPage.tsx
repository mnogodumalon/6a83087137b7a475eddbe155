...
export default function NotizenPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, sitzungenData, protokolleData, mitgliederData] = await Promise.all([
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('notiz_text')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('notizen', 'notiz_text')}
                  {sortKey === 'notiz_text' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('ersteller_vorname')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('notizen', 'ersteller_vorname')}
                  {sortKey === 'ersteller_vorname' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('ersteller_nachname')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('notizen', 'ersteller_nachname')}
                  {sortKey === 'ersteller_nachname' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('notiz_typ')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('notizen', 'notiz_typ')}
                  {sortKey === 'notiz_typ' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/notizen/${record.record_id}`); }}>
                ...
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.notiz_text ?? '—'}</span></TableCell>
                <TableCell>{record.fields.ersteller_vorname ?? '—'}</TableCell>
                <TableCell>{record.fields.ersteller_nachname ?? '—'}</TableCell>
                <TableCell className="text-muted-foreground">{formatDate(record.fields.notiz_datum)}</TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{lookupLabel('notizen', 'notiz_typ', record.fields.notiz_typ?.key) ?? record.fields.notiz_typ?.label ?? '—'}</span></TableCell>
