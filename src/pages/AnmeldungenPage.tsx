...
export default function AnmeldungenPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, sitzungenData] = await Promise.all([
            ...
            <TableRow className="border-b border-input">
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('sitzung')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('anmeldungen', 'sitzung')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('organisation')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('anmeldungen', 'organisation')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('teilnahme')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('anmeldungen', 'teilnahme')}
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/anmeldungen/${record.record_id}`); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{lookupLabel('anmeldungen', 'teilnahme', record.fields.teilnahme?.key) ?? record.fields.teilnahme?.label ?? '—'}</span></TableCell>
