...
import type { Mitglieder, Sitzungen, Anmeldungen, Protokolle, ThemenFeedback, Notizen, CreateMitglieder, CreateSitzungen, CreateAnmeldungen, CreateProtokolle, CreateThemenFeedback, CreateNotizen } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getSitzungen(): Promise<Sitzungen[]> {
    ...
    return enrichLookupFields(records, 'sitzungen');
  }
  static async getSitzungenEntry(id: string): Promise<Sitzungen | undefined> {
    ...
    return enrichLookupFields([record], 'sitzungen')[0];
  }
  ...
  static async getProtokolle(): Promise<Protokolle[]> {
    ...
    return enrichLookupFields(records, 'protokolle');
  }
  static async getProtokolleEntry(id: string): Promise<Protokolle | undefined> {
    ...
    return enrichLookupFields([record], 'protokolle')[0];
  }
  ...
  // --- THEMEN_FEEDBACK ---
  static async getThemenFeedback(): Promise<ThemenFeedback[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.THEMEN_FEEDBACK}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as ThemenFeedback[];
    return enrichLookupFields(records, 'themen_feedback');
  }
  static async getThemenFeedbackEntry(id: string): Promise<ThemenFeedback | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.THEMEN_FEEDBACK}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as ThemenFeedback;
    return enrichLookupFields([record], 'themen_feedback')[0];
  }
  static async createThemenFeedbackEntry(fields: CreateThemenFeedback): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.THEMEN_FEEDBACK}/records`, { fields: cleanFieldsForApi(fields as any, 'themen_feedback') });
    return { ...data, record_id: data.id };
  }
  static async updateThemenFeedbackEntry(id: string, fields: Partial<CreateThemenFeedback>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.THEMEN_FEEDBACK}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'themen_feedback') });
    return { ...data, record_id: data.id };
  }
  static async deleteThemenFeedbackEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.THEMEN_FEEDBACK}/records/${id}`);
  }
