...
import type { Anmeldungen, Sitzungen, LookupValue } from '@/types/app';
...
export type AnmeldungenDialogDefaults = Omit<Anmeldungen['fields'], 'teilnahme'> & {
    teilnahme?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  teilnahme: LOOKUP_OPTIONS['anmeldungen']?.['teilnahme'] ?? [],
};
...
export function AnmeldungenDialog({ open, onClose, onSubmit, defaultValues, recordId, sitzungenList, enablePhotoScan = true, enablePhotoLocation = true }: AnmeldungenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['sitzung', 'vorname', 'nachname', 'email', 'teilnahme'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "sitzung": string | null, // Display name from Sitzungen (see <available-records>)\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "email": string | null, // E-Mail-Adresse\n  "organisation": string | null, // Organisation / Unternehmen\n  "teilnahme": LookupValue | null, // Teilnahme (select one key: "zugesagt" | "abgesagt" | "vielleicht") mapping: zugesagt=Zugesagt, abgesagt=Abgesagt, vielleicht=Vielleicht\n  "anmerkungen": string | null, // Anmerkungen\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'sitzung': (
      <div key="sitzung" className="space-y-1.5">
        <Label htmlFor="sitzung">{fieldLabel('anmeldungen', 'sitzung')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="sitzung"
          placeholder="Zu welcher Sitzung anmelden?"
    ...
    'organisation': (
      <div key="organisation" className="space-y-1.5">
        <Label htmlFor="organisation">{fieldLabel('anmeldungen', 'organisation')}</Label>
        <Input
          id="organisation"
    ...
    'teilnahme': (
      <div key="teilnahme" className="space-y-1.5">
        <Label htmlFor="teilnahme">{fieldLabel('anmeldungen', 'teilnahme')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.teilnahme) === 'zugesagt'}
            onClick={() => setFields(f => ({ ...f, teilnahme: (lookupKey(f.teilnahme) === 'zugesagt' ? undefined : 'zugesagt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.teilnahme) === 'zugesagt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('anmeldungen', 'teilnahme', 'zugesagt') ?? 'Zugesagt'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.teilnahme) === 'abgesagt'}
            onClick={() => setFields(f => ({ ...f, teilnahme: (lookupKey(f.teilnahme) === 'abgesagt' ? undefined : 'abgesagt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.teilnahme) === 'abgesagt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('anmeldungen', 'teilnahme', 'abgesagt') ?? 'Abgesagt'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.teilnahme) === 'vielleicht'}
            onClick={() => setFields(f => ({ ...f, teilnahme: (lookupKey(f.teilnahme) === 'vielleicht' ? undefined : 'vielleicht') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.teilnahme) === 'vielleicht'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('anmeldungen', 'teilnahme', 'vielleicht') ?? 'Vielleicht'}
          </button>
        </div>
        {showErrors && !fields.teilnahme && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'anmerkungen': (
      <div key="anmerkungen" className="space-y-1.5">
        ...
        <Textarea
          id="anmerkungen"
          placeholder="Besondere Wünsche, Diäten, Anfahrtskosten..."
  ...
  const FIELD_LABELS: Record<string, string> = {"sitzung": "Sitzung", "vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "organisation": "Organisation / Unternehmen", "teilnahme": "Teilnahme", "anmerkungen": "Anmerkungen"};
