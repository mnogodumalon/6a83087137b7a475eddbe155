...
export type SitzungenDialogDefaults = Omit<Sitzungen['fields'], 'sitzungsstatus'> & {
    sitzungsstatus?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  sitzungsstatus: LOOKUP_OPTIONS['sitzungen']?.['sitzungsstatus'] ?? [],
};
...
export function SitzungenDialog({ open, onClose, onSubmit, defaultValues, recordId, mitgliederList, enablePhotoScan = true, enablePhotoLocation = true }: SitzungenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['titel', 'datum', 'sitzungsstatus'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "titel": string | null, // Titel der Sitzung\n  "datum": string | null, // YYYY-MM-DDTHH:MM\n  "ort": string | null, // Ort\n  "beschreibung": string | null, // Beschreibung\n  "tagesordnung": string | null, // Tagesordnung\n  "mitglieder": string[] | null, // Display names from Mitglieder, one per referenced record (see <available-records>)\n  "einlade_link": string | null, // Öffentlicher Einlade-Link\n  "einladung_versendet": boolean | null, // Einladung per E-Mail versendet\n  "sitzungsstatus": LookupValue | null, // Status der Sitzung (select one key: "geplant" | "eingeladen" | "durchgefuehrt" | "abgesagt") mapping: geplant=Geplant, eingeladen=Eingeladen, durchgefuehrt=Durchgeführt, abgesagt=Abgesagt\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'titel': (
      <div key="titel" className="space-y-1.5">
        ...
        <Input
          id="titel"
          placeholder="z. B. Quartalstreffen Q3"
          value={fields.titel ?? ''}
    ...
    'datum': (
      <div key="datum" className="space-y-1.5">
        <Label htmlFor="datum">{fieldLabel('sitzungen', 'datum')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="datum"
    ...
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        ...
        <Textarea
          id="beschreibung"
          placeholder="Kurze Zusammenfassung, Hintergrund..."
    ...
    'tagesordnung': (
      <div key="tagesordnung" className="space-y-1.5">
        <Label htmlFor="tagesordnung">{fieldLabel('sitzungen', 'tagesordnung')}</Label>
        <Textarea
          id="tagesordnung"
          placeholder="TOP 1: ..., TOP 2: ..."
          value={fields.tagesordnung ?? ''}
          onChange={e => setFields(f => ({ ...f, tagesordnung: e.target.value }))}
          rows={3}
        />
      </div>
    ),
    'mitglieder': (
      <div key="mitglieder" className="space-y-1.5">
        <Label htmlFor="mitglieder">{fieldLabel('sitzungen', 'mitglieder')}</Label>
        <MultiCombobox
          id="mitglieder"
    ...
    'einladung_versendet': (
      <div key="einladung_versendet" className="space-y-1.5">
        <Label htmlFor="einladung_versendet">{fieldLabel('sitzungen', 'einladung_versendet')}</Label>
        <div className="flex items-center gap-2 pt-1">
          <Checkbox
            id="einladung_versendet"
            checked={!!fields.einladung_versendet}
            onCheckedChange={(v) => setFields(f => ({ ...f, einladung_versendet: !!v }))}
          />
          <Label htmlFor="einladung_versendet" className="font-normal">{fieldLabel('sitzungen', 'einladung_versendet')}</Label>
        </div>
      </div>
    ),
    'sitzungsstatus': (
      <div key="sitzungsstatus" className="space-y-1.5">
        <Label htmlFor="sitzungsstatus">{fieldLabel('sitzungen', 'sitzungsstatus')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.sitzungsstatus) === 'geplant'}
            onClick={() => setFields(f => ({ ...f, sitzungsstatus: (lookupKey(f.sitzungsstatus) === 'geplant' ? undefined : 'geplant') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.sitzungsstatus) === 'geplant'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('sitzungen', 'sitzungsstatus', 'geplant') ?? 'Geplant'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.sitzungsstatus) === 'eingeladen'}
            onClick={() => setFields(f => ({ ...f, sitzungsstatus: (lookupKey(f.sitzungsstatus) === 'eingeladen' ? undefined : 'eingeladen') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.sitzungsstatus) === 'eingeladen'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('sitzungen', 'sitzungsstatus', 'eingeladen') ?? 'Eingeladen'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.sitzungsstatus) === 'durchgefuehrt'}
            onClick={() => setFields(f => ({ ...f, sitzungsstatus: (lookupKey(f.sitzungsstatus) === 'durchgefuehrt' ? undefined : 'durchgefuehrt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.sitzungsstatus) === 'durchgefuehrt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('sitzungen', 'sitzungsstatus', 'durchgefuehrt') ?? 'Durchgeführt'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.sitzungsstatus) === 'abgesagt'}
            onClick={() => setFields(f => ({ ...f, sitzungsstatus: (lookupKey(f.sitzungsstatus) === 'abgesagt' ? undefined : 'abgesagt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.sitzungsstatus) === 'abgesagt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('sitzungen', 'sitzungsstatus', 'abgesagt') ?? 'Abgesagt'}
          </button>
        </div>
        {showErrors && !fields.sitzungsstatus && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
