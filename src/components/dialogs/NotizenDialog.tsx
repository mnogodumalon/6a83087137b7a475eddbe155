...
import type { Notizen, Sitzungen, Protokolle, Mitglieder, LookupValue } from '@/types/app';
...
export type NotizenDialogDefaults = Omit<Notizen['fields'], 'notiz_typ'> & {
    notiz_typ?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  notiz_typ: LOOKUP_OPTIONS['notizen']?.['notiz_typ'] ?? [],
};
...
export function NotizenDialog({ open, onClose, onSubmit, defaultValues, recordId, sitzungenList, protokolleList, mitgliederList, enablePhotoScan = true, enablePhotoLocation = true }: NotizenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['notiz_titel', 'notiz_text'] as const;
  ...
  }), [sitzungenList, protokolleList, mitgliederList, ]);
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "notiz_titel": string | null, // Titel der Notiz\n  "notiz_text": string | null, // Notiztext\n  "ersteller_vorname": string | null, // Vorname Ersteller\n  "ersteller_nachname": string | null, // Nachname Ersteller\n  "notiz_datum": string | null, // YYYY-MM-DD\n  "notiz_typ": LookupValue | null, // Typ der Zuordnung (select one key: "sitzung" | "protokoll" | "mitglied" | "allgemein") mapping: sitzung=Sitzung, protokoll=Protokoll, mitglied=Mitglied, allgemein=Allgemein\n  "sitzung": string | null, // Display name from Sitzungen (see <available-records>)\n  "protokoll": string | null, // Display name from Protokolle (see <available-records>)\n  "mitglied": string | null, // Display name from Mitglieder (see <available-records>)\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["sitzung", "protokoll", "mitglied"]);
        ...
        if (protokollName) {
          const protokollMatch = protokolleList.find(r => matchName(protokollName!, [String(r.fields.titel ?? '')]));
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'notiz_titel': (
      <div key="notiz_titel" className="space-y-1.5">
        ...
        <Input
          id="notiz_titel"
          placeholder="z. B. Follow-up Verträge"
    ...
    'notiz_text': (
      <div key="notiz_text" className="space-y-1.5">
        <Label htmlFor="notiz_text">{fieldLabel('notizen', 'notiz_text')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Textarea
          id="notiz_text"
          placeholder="Was ist wichtig zu merken?"
          value={fields.notiz_text ?? ''}
          onChange={e => setFields(f => ({ ...f, notiz_text: e.target.value }))}
          rows={3}
        />
        {showErrors && !fields.notiz_text && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'ersteller_vorname': (
      <div key="ersteller_vorname" className="space-y-1.5">
        <Label htmlFor="ersteller_vorname">{fieldLabel('notizen', 'ersteller_vorname')}</Label>
        <Input
          id="ersteller_vorname"
          placeholder="z. B. Robert"
          value={fields.ersteller_vorname ?? ''}
          onChange={e => setFields(f => ({ ...f, ersteller_vorname: e.target.value }))}
        />
      </div>
    ),
    'ersteller_nachname': (
      <div key="ersteller_nachname" className="space-y-1.5">
        <Label htmlFor="ersteller_nachname">{fieldLabel('notizen', 'ersteller_nachname')}</Label>
        <Input
          id="ersteller_nachname"
          placeholder="z. B. Wagner"
          value={fields.ersteller_nachname ?? ''}
          onChange={e => setFields(f => ({ ...f, ersteller_nachname: e.target.value }))}
        />
      </div>
    ),
    'notiz_datum': (
      <div key="notiz_datum" className="space-y-1.5">
        <Label htmlFor="notiz_datum">{fieldLabel('notizen', 'notiz_datum')}</Label>
        <DatePicker
          id="notiz_datum"
    ...
    'notiz_typ': (
      <div key="notiz_typ" className="space-y-1.5">
        <Label htmlFor="notiz_typ">{fieldLabel('notizen', 'notiz_typ')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.notiz_typ) === 'sitzung'}
            onClick={() => setFields(f => ({ ...f, notiz_typ: (lookupKey(f.notiz_typ) === 'sitzung' ? undefined : 'sitzung') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.notiz_typ) === 'sitzung'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('notizen', 'notiz_typ', 'sitzung') ?? 'Sitzung'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.notiz_typ) === 'protokoll'}
            onClick={() => setFields(f => ({ ...f, notiz_typ: (lookupKey(f.notiz_typ) === 'protokoll' ? undefined : 'protokoll') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.notiz_typ) === 'protokoll'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('notizen', 'notiz_typ', 'protokoll') ?? 'Protokoll'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.notiz_typ) === 'mitglied'}
            onClick={() => setFields(f => ({ ...f, notiz_typ: (lookupKey(f.notiz_typ) === 'mitglied' ? undefined : 'mitglied') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.notiz_typ) === 'mitglied'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('notizen', 'notiz_typ', 'mitglied') ?? 'Mitglied'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.notiz_typ) === 'allgemein'}
            onClick={() => setFields(f => ({ ...f, notiz_typ: (lookupKey(f.notiz_typ) === 'allgemein' ? undefined : 'allgemein') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.notiz_typ) === 'allgemein'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('notizen', 'notiz_typ', 'allgemein') ?? 'Allgemein'}
          </button>
        </div>
      </div>
    ),
    'sitzung': (
      <div key="sitzung" className="space-y-1.5">
        ...
        <Combobox
          id="sitzung"
          placeholder="Welche Sitzung betrifft die Notiz?"
    ...
    'protokoll': (
      <div key="protokoll" className="space-y-1.5">
        ...
        <Combobox
          id="protokoll"
          placeholder="Welches Protokoll betrifft die Notiz?"
    ...
    'mitglied': (
      <div key="mitglied" className="space-y-1.5">
        ...
        <Combobox
          id="mitglied"
          placeholder="Welches Mitglied betrifft die Notiz?"
  ...
  const FIELD_LABELS: Record<string, string> = {"notiz_titel": "Titel der Notiz", "notiz_text": "Notiztext", "ersteller_vorname": "Vorname Ersteller", "ersteller_nachname": "Nachname Ersteller", "notiz_datum": "Datum der Notiz", "notiz_typ": "Typ der Zuordnung", "sitzung": "Zugehörige Sitzung", "protokoll": "Zugehöriges Protokoll", "mitglied": "Zugehöriges Mitglied"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"sitzung": {"titel": "Titel der Sitzung", "datum": "Datum und Uhrzeit", "ort": "Ort", "beschreibung": "Beschreibung", "tagesordnung": "Tagesordnung", "mitglieder": "Eingeladene Mitglieder", "einlade_link": "Öffentlicher Einlade-Link", "einladung_versendet": "Einladung per E-Mail versendet", "sitzungsstatus": "Status der Sitzung"}, "protokoll": {"protokollstatus": "Status", "titel": "Titel des Protokolls", "sitzung": "Zugehörige Sitzung", "erstellungsdatum": "Erstellungsdatum", "protokollfuehrer_vorname": "Vorname Protokollführer", "protokollfuehrer_nachname": "Nachname Protokollführer", "inhalt": "Protokollinhalt", "beschluesse": "Beschlüsse", "anhang": "Anhang"}, "mitglied": {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "funktion": "Funktion im Gremium", "abteilung": "Abteilung", "status": "Status"}};
    ...
    {createProtokolleOpen && (
      <ProtokolleDialog
        ...
        defaultValues={createProtokolleInitial
          ? ({ titel: createProtokolleInitial } as any)
          : undefined}
