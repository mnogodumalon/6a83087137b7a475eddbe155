...
import type { Protokolle, Sitzungen, LookupValue } from '@/types/app';
...
export type ProtokolleDialogDefaults = Omit<Protokolle['fields'], 'protokollstatus'> & {
    protokollstatus?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  protokollstatus: LOOKUP_OPTIONS['protokolle']?.['protokollstatus'] ?? [],
};
...
export function ProtokolleDialog({ open, onClose, onSubmit, defaultValues, recordId, sitzungenList, enablePhotoScan = true, enablePhotoLocation = true }: ProtokolleDialogProps) {
  ...
  const REQUIRED_FIELDS = ['protokollstatus', 'titel', 'sitzung', 'erstellungsdatum', 'protokollfuehrer_vorname', 'protokollfuehrer_nachname', 'inhalt'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "protokollstatus": LookupValue | null, // Status (select one key: "entwurf" | "freigegeben") mapping: entwurf=Entwurf, freigegeben=Freigegeben\n  "titel": string | null, // Titel des Protokolls\n  "sitzung": string | null, // Display name from Sitzungen (see <available-records>)\n  "erstellungsdatum": string | null, // YYYY-MM-DD\n  "protokollfuehrer_vorname": string | null, // Vorname Protokollführer\n  "protokollfuehrer_nachname": string | null, // Nachname Protokollführer\n  "inhalt": string | null, // Protokollinhalt\n  "beschluesse": string | null, // Beschlüsse\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["sitzung"]);
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'protokollstatus': (
      <div key="protokollstatus" className="space-y-1.5">
        <Label htmlFor="protokollstatus">{fieldLabel('protokolle', 'protokollstatus')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.protokollstatus) === 'entwurf'}
            onClick={() => setFields(f => ({ ...f, protokollstatus: (lookupKey(f.protokollstatus) === 'entwurf' ? undefined : 'entwurf') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.protokollstatus) === 'entwurf'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('protokolle', 'protokollstatus', 'entwurf') ?? 'Entwurf'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.protokollstatus) === 'freigegeben'}
            onClick={() => setFields(f => ({ ...f, protokollstatus: (lookupKey(f.protokollstatus) === 'freigegeben' ? undefined : 'freigegeben') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.protokollstatus) === 'freigegeben'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('protokolle', 'protokollstatus', 'freigegeben') ?? 'Freigegeben'}
          </button>
        </div>
        {showErrors && !fields.protokollstatus && (
    ...
    'titel': (
      <div key="titel" className="space-y-1.5">
        <Label htmlFor="titel">{fieldLabel('protokolle', 'titel')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="titel"
          placeholder="z. B. Protokoll Q3-Quartal"
          value={fields.titel ?? ''}
    ...
    'protokollfuehrer_vorname': (
      <div key="protokollfuehrer_vorname" className="space-y-1.5">
        <Label htmlFor="protokollfuehrer_vorname">{fieldLabel('protokolle', 'protokollfuehrer_vorname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="protokollfuehrer_vorname"
          placeholder="z. B. Klaus"
          value={fields.protokollfuehrer_vorname ?? ''}
          onChange={e => setFields(f => ({ ...f, protokollfuehrer_vorname: e.target.value }))}
          required
        />
        {showErrors && !fields.protokollfuehrer_vorname && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'protokollfuehrer_nachname': (
      <div key="protokollfuehrer_nachname" className="space-y-1.5">
        <Label htmlFor="protokollfuehrer_nachname">{fieldLabel('protokolle', 'protokollfuehrer_nachname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="protokollfuehrer_nachname"
          placeholder="z. B. Schmidt"
          value={fields.protokollfuehrer_nachname ?? ''}
          onChange={e => setFields(f => ({ ...f, protokollfuehrer_nachname: e.target.value }))}
          required
        />
        {showErrors && !fields.protokollfuehrer_nachname && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'inhalt': (
      <div key="inhalt" className="space-y-1.5">
        <Label htmlFor="inhalt">{fieldLabel('protokolle', 'inhalt')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Textarea
          id="inhalt"
          placeholder="Auszüge aus der Sitzung, Diskussionspunkte, Ergebnisse..."
          value={fields.inhalt ?? ''}
          onChange={e => setFields(f => ({ ...f, inhalt: e.target.value }))}
          rows={3}
        />
        {showErrors && !fields.inhalt && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'beschluesse': (
      <div key="beschluesse" className="space-y-1.5">
        ...
        <Textarea
          id="beschluesse"
          placeholder="Gefasste Beschlüsse, Abstimmungsergebnisse..."
    ...
    'anhang': (
      <div key="anhang" className="space-y-1.5">
        <Label htmlFor="anhang">{fieldLabel('protokolle', 'anhang')}</Label>
        {fields.anhang ? (
  ...
  const FIELD_LABELS: Record<string, string> = {"protokollstatus": "Status", "titel": "Titel des Protokolls", "sitzung": "Zugehörige Sitzung", "erstellungsdatum": "Erstellungsdatum", "protokollfuehrer_vorname": "Vorname Protokollführer", "protokollfuehrer_nachname": "Nachname Protokollführer", "inhalt": "Protokollinhalt", "beschluesse": "Beschlüsse", "anhang": "Anhang"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"sitzung": {"titel": "Titel der Sitzung", "datum": "Datum und Uhrzeit", "ort": "Ort", "beschreibung": "Beschreibung", "tagesordnung": "Tagesordnung", "mitglieder": "Eingeladene Mitglieder", "einlade_link": "Öffentlicher Einlade-Link", "einladung_versendet": "Einladung per E-Mail versendet", "sitzungsstatus": "Status der Sitzung"}};
