...
- **`file` fields cannot go public AT ALL** — not in a list (reading) and not
  in a create (uploading): the `/files` endpoint is not grantable, so the
  service rejects the ENTIRE page, and only after the deploy. Leave every
  file field out, even when a brief names one. A picture reaches a public
  page only through a `string/url` field holding an externally hosted image
  URL; real attachments stay on the internal dashboard.
